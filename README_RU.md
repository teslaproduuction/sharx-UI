...
Добавление, привязка к inbound и обслуживание узлов выполняется **в веб-панели** (например, разделы **Ноды** и **География**). Скрипт `install_ru.sh` в дистрибутиве SharX устанавливает только панель и стек с БД.

Если нужно развернуть **отдельный** worker-контейнер ноды на другом сервере (образ, порты, `NODE_TLS_*`), см. [node/README.md](node/README.md) в этом репозитории.
...
**TLS для отдельного worker-узла (не панели):** переменные `NODE_TLS_CERT_FILE` и `NODE_TLS_KEY_FILE` используются только в `docker-compose` **образа ноды**; см. [node/README.md](node/README.md).
