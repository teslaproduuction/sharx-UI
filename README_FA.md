...
افزودن نود، اتصال به inbound و مدیریت روزمره **در پنل وب** انجام می‌شود. اسکریپت `install.sh` در بسته‌ی نصب SharX فقط پنل و دیتابیس را نصب می‌کند.

برای اجرای **جداگانه** کانتینر worker نود روی سرور دیگر (ایمیج، پورت، `NODE_TLS_*`)، به [node/README.md](node/README.md) در همین مخزن مراجعه کنید.
...
**TLS سرویس worker نود (نه پنل):** متغیرهای `NODE_TLS_CERT_FILE` و `NODE_TLS_KEY_FILE` فقط در `docker-compose` **نود** استفاده می‌شوند؛ مراجع: [node/README.md](node/README.md).
