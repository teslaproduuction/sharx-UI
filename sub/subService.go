...
func (s *SubService) genMieruLinkWithClient(inbound *model.Inbound, client *model.ClientEntity) string {
	...
	if pwd == "" {
		...
		em := strings.ToLower(strings.TrimSpace(client.Name))
	...
	return s.buildMieruURI(inbound, client.Name, pwd, raw.Transport, raw.Multiplexing, raw.MTU)
}
...
func (s *SubService) genAnyTLSLinkWithClient(inbound *model.Inbound, client *model.ClientEntity) string {
	...
	pwd := strings.TrimSpace(client.Password)
	if pwd == "" {
		pwd, _ = extractInlineCredentialsForEmail(inbound.Settings, client.Name)
	}
	return s.buildAnyTLSURI(inbound, pwd)
}
...
func (s *SubService) genNaiveLinkWithClient(inbound *model.Inbound, client *model.ClientEntity) string {
	...
	return s.buildNaiveURI(inbound, client.Name, pwd)
}
...
func (s *SubService) genTuicLinkWithClient(inbound *model.Inbound, client *model.ClientEntity) string {
	...
	if pwd == "" || uuid == "" {
		...
		inlinePwd, inlineUUID := extractInlineCredentialsForEmail(inbound.Settings, client.Name)
		if pwd == "" {
			pwd = inlinePwd
		}
		if uuid == "" {
